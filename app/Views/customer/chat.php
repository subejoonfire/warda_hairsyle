...
<div class="max-w-4xl mx-auto">
        ...
        <div class="p-4 md:p-6 border-b border-gray-200">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                <div class="mb-2 md:mb-0">
                    <h1 class="text-xl md:text-2xl font-bold text-gray-800">Chat dengan Admin</h1>
                    <p class="text-sm md:text-base text-gray-600">Konsultasi atau tanya jawab dengan tim admin kami</p>
                </div>
                <div class="flex items-center space-x-2">
                    <div class="w-3 h-3 bg-green-500 rounded-full"></div>
                    <span class="text-sm text-gray-600">Admin Online</span>
                </div>
            </div>
        </div>

        <!-- Chat Messages -->
        <div id="chat-messages" class="h-80 md:h-96 overflow-y-auto p-4 md:p-6 space-y-4">
            <?php if (empty($chats)): ?>
                <div class="text-center py-8">
                    <div class="text-gray-400 mb-4">
                        <i class="fas fa-comments text-4xl"></i>
                    </div>
                    <h3 class="text-lg font-semibold text-gray-600 mb-2">Belum ada pesan</h3>
                    <p class="text-gray-500">Mulai percakapan dengan admin untuk konsultasi atau tanya jawab</p>
                </div>
            <?php else: ?>
                <?php foreach ($chats as $chat): ?>
                    <div class="flex <?= $chat['sender_type'] === 'customer' ? 'justify-end' : 'justify-start' ?>">
                        <div class="max-w-[85%] md:max-w-xs lg:max-w-md px-3 md:px-4 py-2 rounded-lg <?= $chat['sender_type'] === 'customer' ? 'bg-accent text-white' : 'bg-gray-100 text-gray-800' ?>">
                            <div class="text-sm">
            ...
            <form id="chat-form" class="flex flex-col md:flex-row space-y-3 md:space-y-0 md:space-x-4">
                <div class="flex-1">
                    <textarea id="message-input" 
                              class="w-full px-3 md:px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-accent focus:border-accent resize-none"
                              rows="2"
                              placeholder="Ketik pesan Anda di sini..."
                              maxlength="500"></textarea>
                </div>
                <button type="submit" 
                        class="bg-accent text-white px-4 md:px-6 py-2 rounded-lg hover:bg-yellow-600 transition duration-300 flex items-center justify-center">
    ...
    <!-- Quick Messages -->
    <div class="mt-6 bg-white rounded-lg shadow-lg p-4 md:p-6">
        <h3 class="text-lg font-semibold mb-4">Pesan Cepat</h3>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
            <button class="quick-message bg-gray-100 hover:bg-gray-200 text-gray-800 px-4 py-2 rounded-lg text-sm transition duration-300">
