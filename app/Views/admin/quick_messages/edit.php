...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Edit Quick Message</h1>
            <p class="text-gray-600">Edit response otomatis untuk customer chat</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow p-6">
        <form action="<?= base_url('admin/quick-messages/edit/' . $quick_message['id']) ?>" method="POST" id="quickMessageForm">
            <?= csrf_field() ?>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" id="keyword" name="keyword" 
                               value="<?= old('keyword', $quick_message['keyword']) ?>" 
                    ...
                    <div>
                        ...
                        <input type="number" id="sort_order" name="sort_order" 
                               value="<?= old('sort_order', $quick_message['sort_order']) ?>" 
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-accent focus:border-accent"
                               placeholder="0">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <select id="response_type" name="response_type" 
                            ...
                            <option value="static" <?= (old('response_type', $quick_message['response_type']) === 'static') ? 'selected' : '' ?>>Static (Teks tetap)</option>
                            <option value="dynamic" <?= (old('response_type', $quick_message['response_type']) === 'dynamic') ? 'selected' : '' ?>>Dynamic (Dari database)</option>
                            <option value="template" <?= (old('response_type', $quick_message['response_type']) === 'template') ? 'selected' : '' ?>>Template (Campuran)</option>
                        </select>
                    ...
                    <div id="response_source_group" style="display: none;">
                        ...
                        <select id="response_source" name="response_source" 
                            ...
                            <?php foreach ($response_sources as $key => $value) : ?>
                                <option value="<?= $key ?>" <?= (old('response_source', $quick_message['response_source']) === $key) ? 'selected' : '' ?>><?= $value ?></option>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <div id="response_template_group" style="display: none;">
                        ...
                        <textarea id="response_template" name="response_template" rows="8" 
                                  class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-accent focus:border-accent"
                                  placeholder="Masukkan template response..."><?= old('response_template', $quick_message['response_template']) ?></textarea>
                        <p class="text-xs text-gray-500 mt-1">
